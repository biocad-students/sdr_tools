...
class Rotamer(val atoms : Seq[PDBAtomInfo], val ca : GeometryVector) {
  /*
  val center : GeometryVector = atoms.foldLeft(Vector3d(0, 0, 0)) ({case (result, atom) => {
...
class SimplifiedAminoAcid(val atoms : Seq[PDBAtomInfo]) {
  val ca : GeometryVector = atoms.find{_.atom == "CA"} match{
  ...
  */
  ...
  var rotamer : Rotamer = new Rotamer(atoms.filterNot{s=>Seq("N", "H").contains(s.atom)}, ca)//todo: check this
