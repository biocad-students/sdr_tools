...
class DTGraph:
    ...
    def get_neighbours(self, start_nodes):
        ...
        #print len(result)
        return result
    def get_start_simplices(self, triangles, check_edge, distance_func, check_simplex):
        print self.surface[1].simplices[1, :]
        return np.unique(np.asarray([
            [
                i for i in range(len(self.surface[1].simplices))
                if len(np.setdiff1d(self.surface[1].simplices[i], triangle)) == 1
            ]
